...
par(mfrow=c(3, 1))
plotEMmodel(x, m, s, p, bestPeak, xlim=range(0, 6), ylim=range(0, .6))

# Resampling method
xnorm <- EMnormalize(x, xlim=range(0, 6), ylim=range(0, .6))

# True groups
plotEMmodel(x, 1:4, rep(0.5, 4), p.init, which.max(p.init), xlim=range(0, 6), ylim=range(0, .6))
par(op)
